...
if float(len(student_list))%5==0:
    max=int(len(student_list)/5)
else:
    max=int(len(student_list)//5)+1

for i in range(1,(max+1)):
    print('Group',i)
    random.shuffle(student_list)
    list1=random.sample(student_list,k=5)
    for x in list1:
        student_list.remove(x)
        print('Student ID {0}'.format(x))
    y=random.choice(case_list)
    case_list.remove(y)
    print('Assigned {0} \n==============='.format(y))